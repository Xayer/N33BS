...
void set_NLBL ( void ) {
   mySerCmd.AddCmd ( ( char * ) "LEDBL" , SERIALCMD_FROMALL, NULL );
   sendOK();
}

void set_YLBL ( void ) {
   mySerCmd.AddCmd ( ( char * ) "LEDBL" , SERIALCMD_FROMALL, set_LEDBL );
   sendOK();
}
...
void setup() {
   ...
   //
#ifdef __AVR__
   mySerCmd.AddCmd (  ( char * ) "LEDON"  , SERIALCMD_FROMALL, set_LEDON ); // BUILTIN  LED ON
   mySerCmd.AddCmd (  ( char * ) "LEDOF"  , SERIALCMD_FROMALL, set_LEDOF ); // BUILTIN  LED OFF
   mySerCmd.AddCmd ( ( char * ) "LEDBL" , SERIALCMD_FROMALL, set_LEDBL ); // BUILTIN  LED BLINK, period ms in parameter
   mySerCmd.AddCmd ( ( char * ) "SETNB" , SERIALCMD_FROMALL, set_NLBL );  // DISABLE  LEDBL command
   mySerCmd.AddCmd (  ( char * ) "SETYB"  , SERIALCMD_FROMALL, set_YLBL );  // REENABLE LEDBL command
   //
   mySerCmd.Print (  ( char * ) "INFO: Program running on AVR ... \r\n" );
#else
   mySerCmd.AddCmd ( ( char * ) "LEDON", SERIALCMD_FROMALL, set_LEDON );
   mySerCmd.AddCmd ( ( char * ) "LEDOF", SERIALCMD_FROMALL, set_LEDOF );
   mySerCmd.AddCmd ( ( char * ) "LEDBL", SERIALCMD_FROMALL, set_LEDBL );
   mySerCmd.AddCmd ( ( char * ) "SETNB", SERIALCMD_FROMALL, set_NLBL );
   mySerCmd.AddCmd ( ( char * ) "SETYB", SERIALCMD_FROMALL, set_YLBL );
   //
...
#endif
      if ( ret == false ) {
         mySerCmd.Print ( ( char * ) "ERROR: ret is false \r\n" );
